...
# Compare protoc version with the wanted one
if ! [ "$protoc_v" == "$protoc_version" ]; then
   ...
   if [[ "$os_type" == "Linux" ]]; then
      ...
      case $os_arch in
         (amd64|x86_64|arm64)   protoc_url_file="protoc-${protoc_version}-osx-x86_64.zip" ;;
...
# Define protoc-gen-grpc-java version
if [[ "$os_type" == "Linux" ]]; then
   case $os_arch in
   ...
   case $os_arch in
     (amd64|x86_64|arm64)   protoc_grpc_url_file="protoc-gen-grpc-java-${protoc_grpc_version}-osx-x86_64.exe" ;;
     (*) echo "os arch not supported $os_arch" && exit 0 ;;
   esac
fi

# Get protoc-gen-grpc-java
...
# Move protoc-gen-grpc-java version to a generic name
if [ -f "$protoc_grpc_url_file" ]; then
      mv "${protoc_grpc_url_file}" "protoc-gen-grpc-java"
fi
